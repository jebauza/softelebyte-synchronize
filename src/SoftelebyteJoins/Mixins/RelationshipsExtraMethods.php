...
{
    /**
     * Perform the JOIN clause for eloquent softelebyte joins.
     */
    public function performJoinForEloquentSoftelebyteJoins()
    {
        return function (
            string $relationName,
            $builder,
            ...
            if ($this instanceof BelongsToMany) {
                return $this->performJoinForEloquentSoftelebyteJoinsForBelongsToMany($builder, $joinType, $callback, $alias, $disableExtraConditions, $relationName);
            } elseif ($this instanceof MorphOneOrMany) {
                $this->performJoinForEloquentSoftelebyteJoinsForMorph($builder, $joinType, $callback, $alias, $disableExtraConditions, $relationName);
            } elseif ($this instanceof HasMany || $this instanceof HasOne) {
                return $this->performJoinForEloquentSoftelebyteJoinsForHasMany($builder, $joinType, $callback, $alias, $disableExtraConditions, $relationName);
            } elseif ($this instanceof HasManyThrough) {
                return $this->performJoinForEloquentSoftelebyteJoinsForHasManyThrough($builder, $joinType, $callback, $alias, $disableExtraConditions, $relationName);
            } else {
                return $this->performJoinForEloquentSoftelebyteJoinsForBelongsTo($builder, $joinType, $callback, $alias, $disableExtraConditions, $relationName);
            }
        };
    }
     ...
     */
    protected function performJoinForEloquentSoftelebyteJoinsForBelongsTo()
    {
        return function (
            $query,
            $joinType,
            ...
            $parentRelationName = substr($relationName, 0, strrpos($relationName, '.'));
            $joinedTable = $alias ?? $this->getTableOrAliasForRelation($relationName, $query, $this->query->getModel()->getTable());
            $parentTable = $this->getTableOrAliasForRelation($parentRelationName, $query, $this->parent->getTable());

            $query->{$joinType}($this->query->getModel()->getTable(), function (SoftelebyteJoinClause $join) use ($callback, $joinedTable, $parentTable, $alias, $disableExtraConditions) {
                if ($alias) {
                    $join->md5Alias = $alias;
                    $join->as($alias);
                }
     ...
     */
    protected function performJoinForEloquentSoftelebyteJoinsForBelongsToMany()
    {
        return function ($builder, $joinType, $callback = null, $alias = null, bool $disableExtraConditions = false, string $relationName = '') {
            [$alias1, $alias2] = $alias;
            $parentRelationName = substr($relationName, 0, strrpos($relationName, '.'));
            $joinedTable = $alias1 ?: $this->getTableOrAliasForRelation($relationName, $builder, $this->getTable());
            $parentTable = $this->getTableOrAliasForRelation($parentRelationName, $builder, $this->parent->getTable()) ?? $this->parent->getTable();
            $nextTable = $alias2 ?: $this->getTableOrAliasForRelation($relationName, $builder, $this->getModel()->getTable()) ?? $this->getModel()->getTable();

            $builder->{$joinType}($this->getTable(), function ($join) use ($callback, $joinedTable, $parentTable, $alias1) {
                if ($alias1) {
                    $join->md5Alias = $alias1;
                    $join->as($alias1);
                }
            ...
            $builder->{$joinType}($this->getModel()->getTable(), function ($join) use ($callback, $joinedTable, $nextTable, $alias2, $disableExtraConditions) {
                if ($alias2) {
                    $join->md5Alias = $alias2;
                    $join->as($alias2);
                }

                $join->on(
                    "{$nextTable}.{$this->getModel()->getKeyName()}",
                    '=',
     ...
     */
    protected function performJoinForEloquentSoftelebyteJoinsForMorph()
    {
        return function ($builder, $joinType, $callback = null, $alias = null, bool $disableExtraConditions = false, string $relationName = '') {
     ...
     */
    protected function performJoinForEloquentSoftelebyteJoinsForHasMany()
    {
        return function ($builder, $joinType, $callback = null, $alias = null, bool $disableExtraConditions = false, string $relationName = '') {
            $parentRelationName = substr($relationName, 0, strrpos($relationName, '.'));
            $joinedTable = $alias ?: $this->getTableOrAliasForRelation($relationName, $builder, $this->query->getModel()->getTable());
            $parentTable = $this->getTableOrAliasForRelation($parentRelationName, $builder, $this->parent->getTable());
            ...
            $builder->{$joinType}($this->query->getModel()->getTable(), function ($join) use ($callback, $joinedTable, $parentTable, $alias, $disableExtraConditions) {
                ...
                $join->on(
                    "{$joinedTable}.{$this->getForeignKeyName()}",
                    '=',
     ...
     */
    protected function performJoinForEloquentSoftelebyteJoinsForHasManyThrough()
    {
        return function ($builder, $joinType, $callback = null, $alias = null, bool $disableExtraConditions = false, string $relationName = '') {
            [$alias1, $alias2] = $alias;
            ...
            $builder->{$joinType}($this->getThroughParent()->getTable(), function (SoftelebyteJoinClause $join) use ($callback, $throughTable, $alias1, $disableExtraConditions) {
                if ($alias1) {
                    $join->md5Alias = $alias1;
                    $join->as($alias1);
                }
            ...
            $builder->{$joinType}($this->getModel()->getTable(), function (SoftelebyteJoinClause $join) use ($callback, $throughTable, $farTable, $alias1, $alias2) {
                if ($alias2) {
                    $join->md5Alias = $alias2;
                    $join->as($alias2);
                }
    ...
    /**
     * Perform the "HAVING" clause for eloquent softelebyte joins.
     */
    public function performHavingForEloquentSoftelebyteJoins()
    {
    ...
    public function getTableOrAliasForRelation()
    {
        return function (string $relationName, $query, $model) {
            /** @var $query SoftelebyteJoins */
            $alias = $query->getSoftelebyteJoinAliases($relationName, $model) ?? $model;
            return $alias;
        };
    }
    ...
    {
        return function (SoftelebyteJoinClause $join) {
    ...
    public function shouldNotApplyExtraCondition()
    {
        return function ($condition) {
            $key = $this->getSoftelebyteJoinExistenceCompareKey();
    ...
    public function getSoftelebyteJoinExistenceCompareKey()
    {
        return function () {
