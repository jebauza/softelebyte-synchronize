...
class SoftelebyteJoinClause extends JoinClause
{
    /**
    ...
    protected function useTableAliasInConditions()
    {
        ...
        $this->wheres = collect($this->wheres)->filter(function ($where) {
            ...
            if (Str::contains($where['first'], $table) && Str::contains($where['second'], $table)) {
                ...
                $where['first'] = str_replace($table . '.' . $key, $this->md5Alias . '.' . $key, $where['first']);
                $where['second'] = str_replace($table . '.' . $key, $this->md5Alias . '.' . $key, $where['second']);
            } else {
                $where['first'] = str_replace($table, $this->md5Alias, $where['first']);
                $where['second'] = str_replace($table, $this->md5Alias, $where['second']);
            }
    ...
    {
        if ($this->md5Alias && Str::contains($columns, $this->tableName)) {
            $columns = str_replace("{$this->tableName}.", "{$this->md5Alias}.", $columns);
        }
    ...
    {
        if ($this->md5Alias && is_string($column) && Str::contains($column, $this->tableName)) {
            $column = str_replace("{$this->tableName}.", "{$this->md5Alias}.", $column);
        }
